...
{
    public class BookDetails_DAL
    {
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["bookDB"].ConnectionString);

        // Method call save the book record into db
        public Int32 SaveBookDetails(BEL_BookApp.BookDetails_BEL objBEL)
        {
            int result;
            try
            {
        ...
        // Method return the record should be deleted from Database
        ...
        //Method call delete book record from databas

        public Int32 DeleteBookRecord(BookDetails_BEL objBEL)
        {
            int result;
            try
            {
        ...
        public Int32 UpdateBookRecord(BookDetails_BEL objBEL)
        {
            ...
            int result;
            try {
                SqlCommand cmd = new SqlCommand("UpdateBookRecords_SP", con);
                 cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@BookId", objBEL.BookId);
                cmd.Parameters.AddWithValue("@BookName",objBEL.BookName);
                cmd.Parameters.AddWithValue("@Author",objBEL.Author);
